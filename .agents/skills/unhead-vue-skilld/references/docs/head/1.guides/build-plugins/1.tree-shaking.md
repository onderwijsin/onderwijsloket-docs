...
The transform parses each file with oxc-parser and walks the AST looking for `ExpressionStatement` nodes that call one of the server composable names. Matching statements are removed from the source using magic-string, preserving sourcemaps.
