...
description: "Adapt production Directus query patterns for FAQs, articles, and institutions."
---

These examples recreate API calls used on [onderwijsloket.com](http://onderwijsloket.com). Adapt the requested fields and filters to your UI; the complete snippets also live in our [code snippets repository](https://github.com/onderwijsin/onderwijsloket-examples/tree/main).
...
### Request the article and its dependencies
 ...
 */
const bodyWithInjectedData = injectData(item.editor_nodes as ArticlesEditorNode[], item.body);
```
...
```typescript
function getNestedRelationships<T>(item: unknown, config: { key: string }): T[] {
...
const processedItem = {
  ...
  documents: getNestedRelationships<Document & { asset: Partial<DirectusFile> }>(item, {
    key: "documents"
  }),
...
## Fetch an institution and its programmes
