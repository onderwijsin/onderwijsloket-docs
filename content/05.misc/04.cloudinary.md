...
[YouTube video player](https://www.youtube.com/embed/7qujZvoaxS4)
...
- `{asset_type}` is typically `image`, `video`, or `raw` (for documents and other files).

- `{public_id}` is `onderwijsloket-directus/production/{ID assigned by Directus}`.
