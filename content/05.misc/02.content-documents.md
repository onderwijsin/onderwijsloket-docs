...
When working with content collections, you will often encounter `body` and `editor_nodes` fields. These fields contain [Tiptap editor instances](https://tiptap.dev/) enriched with relational data for custom nodes. This article describes how to process and render this data.
...
const relatedSectors = [
  ...
  "description",
  "date_created",
] satisfies QueryFields<Schema, Sector>;
...
const injectData = (
  ...
  function walk(
    ...
    if (
      node.type &&
      RELATION_NODE_TYPES.includes(node.type) &&
      node.attrs?.id
    ) {
      const related = availableData.find(
        (n) => n[primaryKeyField] === node.attrs!.id,
      );

      if (related?.[itemField]) {
        const itemData = deepClone(related[itemField]) as Record<
          string,
          unknown
        >;
    ...
    if (node.type === "text" && Array.isArray(node.marks)) {
      node.marks = node.marks.map((mark) => {
        ...
        const related = availableData.find(
          (n) => n[primaryKeyField] === mark.attrs!.id,
        );
...
const data = injectData(
  response.editor_nodes as ArticlesEditorNode[],
  response.body,
);
```
